...
def build_transformer(src_vocab_size: int, tgt_vocab_size: int, src_seq_len: int, tgt_seq_len: int, d_model: int = 512,
                      N: int = 6, h: int = 8, dropout: float = config.dropout,
                      attn_dropout: float = config.attn_dropout, d_ff: int = 2048) -> Transformer:
    ...
    encoder_blocks = []
    for _ in range(N):
        encoder_self_attention_block = MultiHeadAttentionBlock(d_model, h, attn_dropout)
    ...
    decoder_blocks = []
    for _ in range(N):
        decoder_self_attention_block = MultiHeadAttentionBlock(d_model, h, attn_dropout)
        decoder_cross_attention_block = MultiHeadAttentionBlock(d_model, h, attn_dropout)
        feed_forward_block = FeedForwardBlock(d_model, d_ff, dropout)
